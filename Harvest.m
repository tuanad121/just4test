...
boundary_f0_list =...
  f0_floor_adjusted * 2.0 .^...
  ((1 : ceil(log2(f0_ceil_adjusted / f0_floor_adjusted) *...
...
raw_f0_candidates = CalculateCandidates(...
...
[f0_candidates, f0_candidates_score] =...
  RemoveUnreliableCandidates(f0_candidates, f0_candidates_score);

[connected_f0, vuv] =...
  FixF0Contour(f0_candidates, f0_candidates_score);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [new_f0_candidates, new_f0_candidates_score] =...
  RemoveUnreliableCandidates(f0_candidates, f0_candidates_score)
new_f0_candidates = f0_candidates;
new_f0_candidates_score = f0_candidates_score;
threshold = 0.05;
...
for i = 0 : number_of_candidates - 1
  st1 = max(-(i - n) + 1, 1);
  ed1 = min(-(i - n), 0);
  new_f0_candidates((1 : max_candidates) + i * max_candidates, st1 : end + ed1) =...
    f0_candidates(1 : max_candidates, -ed1 + 1 : end - (st1 - 1));
end;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function raw_f0_candidates = CalculateCandidates(number_of_frames,...
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function f0_candidates = CalculateRawEvent(boundary_f0,...
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function interpolated_f0 = GetF0Candidates(negative_zero_cross,...
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [f0, vuv] = FixF0Contour(f0_candidates, f0_candidates_score)
...
function f0_base = SearchF0Base(f0_candidates, f0_candidates_score)
f0_base = f0_candidates(1, :) * 0;
for i = 1 : length(f0_base)
  [~, max_index] = max(f0_candidates_score(:, i));
...
function f0_step3 =...
  FixStep3(f0_step2, f0_candidates, allowed_range, f0_candidates_score)
f0_step3 = f0_step2;
...
if count > 0
  f0_step3 =...
    MergeF0(multi_channel_f0, range, f0_candidates, f0_candidates_score);
end;
...
for i = 2 : number_of_channels
  ...
  if range(sorted_order(i), 1) - range(sorted_order(1), 2) > 0
    ...
    [f0, range(sorted_order(1), 2)] =...
      ...
      range(sorted_order(i), 2), f0_candidates, f0_candidates_score);
  end;
end;
...
for i = st2 : ed1
  score1 = score1 +...
    SerachScore(f0_1(i), f0_candidates(:, i), f0_candidates_score(:, i));
  score2 = score2 +...
    SerachScore(f0_2(i), f0_candidates(:, i), f0_candidates_score(:, i));
end;
if score1 > score2
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function score = SerachScore(f0, f0_candidates, f0_candidates_score)
score = 0;
...
for i = 1 : length(boundary_list) / 2 - 1
  ...
  tmp0 = f0_step3(boundary_list(2 * i)) + 1;
  tmp1 = f0_step3(boundary_list((2 * i) + 1)) - 1;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [new_f0_candidates, f0_candidates_score] = RefineCandidates(x, fs,...
...
% First-aid treatment
index_raw = round((current_time + base_time) * fs + 0.001);
...
max_trim = min(floor(fs / 2 / current_f0), 6); % with safe guard
trim_index = (1 : max_trim)';
index_list_trim = round(current_f0 * fft_size / fs * trim_index) + 1;
...
variation = abs(((fixp_list ./ trim_index) - current_f0) ./ current_f0);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function smoothed_f0 = SmoothF0(f0)
...
for i = 1 : length(boundary_list) / 2
  tmp_f0_contour = FilterF0(multi_channel_f0(i, :),...
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function smoothed_f0 = FilterF0(f0_contour, st, ed, b, a)
smoothed_f0 = f0_contour;
